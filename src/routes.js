...
router.get("/fusamiebg/consulta/:cedula?", async (req, res) => {
  ...
  try {
    ...
    const query = await specificQuery({ sqlQuery, table: "f.", db: 1 });
    ...
    if (checkQuery > 0) {
      ...
      */
      ...
      const query_obrero = await specificQuery({ sqlQuery: sqlQuery_obrero, table: "f.", db: 1 });
...
router.get("/hoja_vida/consulta_foto/:cedula", async (req, res) => {
  ...
  try {
    ...
    const where = IS_ONLY_LN
      ? `f.cedula_identidad=${cedula} and t.ano=${CURRENT_YEAR} [condition_ext]`
      : `f.cedula_identidad=${cedula} and t.ano=${CURRENT_YEAR} [condition_ext]`;
    // const where = IS_ONLY_LN
    //   ? `f.cedula_identidad=${cedula} and t.ano=${CURRENT_YEAR} and f.condicion_actividad_ficha=1 and hn.clasificacion_personal not in (7,8,13,3,4,6,15,9,10,11,12,13,14) [condition_ext]`
...
router.get("/hoja_vida/consulta_dep/:cod_dep", async (req, res) => {
  ...
  const getCondition = () => {
    ...
    if (cod_dep == 1006) {
      return IS_ONLY_LN
        ? `f.cod_dep=${cod_dep} and f.cod_tipo_nomina in (1,2) and hn.clasificacion_personal not in (7,8,13,3,4,6,15,9,10,11,12,13,14)`
        : `f.cod_dep=${cod_dep} and f.cod_tipo_nomina not in (7,8,9,10,11,12) and hn.clasificacion_personal not in (7,8,13,3,4,6,15,9,10,11,13,14)`;
    }
    if (cod_dep == 1007) {
      return IS_ONLY_LN
        ? `f.cod_dep=${cod_dep} and f.cod_tipo_nomina in (1,2) and hn.clasificacion_personal not in (7,8,13,3,4,6,15,9,10,11,12,13,14)`
    ...
    if (cod_dep == 1010) {
      return IS_ONLY_LN
        ? `f.cod_dep=${cod_dep} and f.cod_tipo_nomina in (1,2) and hn.clasificacion_personal not in (7,8,13,3,4,6,15,9,10,11,12,13,14)`
        : `f.cod_dep=${cod_dep} and f.cod_tipo_nomina not in (6,9,10,11) and hn.clasificacion_personal not in (7,8,13,3,4,6,15,9,10,11,12,13,14)`;
    }
    if (cod_dep == 1011) {
      return IS_ONLY_LN
        ? `f.cod_dep=${cod_dep} and f.cod_tipo_nomina in (1,2) and hn.clasificacion_personal not in (7,8,13,3,4,6,15,9,10,11,12,13,14)`
    ...
    if (cod_dep == 1040) {
      return IS_ONLY_LN
        ? `f.cod_dep=${cod_dep} and f.cod_tipo_nomina in (1, 2, 3) and hn.clasificacion_personal not in (7,8,13,3,4,6,15,9,10,11,12,13,14)`
        : `f.cod_dep=${cod_dep} and hn.clasificacion_personal not in (7,8,13,3,4,6,15,9,10,11,13,14)`;
    }
    if (cod_dep == 1041) {
      return IS_ONLY_LN
        ? `f.cod_dep=${cod_dep} and f.cod_tipo_nomina in (1, 2) and hn.clasificacion_personal not in (7,8,13,3,4,6,15,9,10,11,12,13,14)`
    ...
    if (cod_dep == 1046) {
      return IS_ONLY_LN ? `f.cod_dep=${cod_dep} and f.cod_tipo_nomina in (1,2) and hn.clasificacion_personal not in (7,8,13,3,4,6,15,9,10,11,12,13,14)` : `f.cod_dep=${cod_dep}`;
    }
    return IS_ONLY_LN
      ? `f.cod_dep=${cod_dep} and f.cod_tipo_nomina in (1,2) and hn.clasificacion_personal not in (7,8,13,3,4,6,15,9,10,11,12,13,14)`
  ...
  try {
    ...
    (select devolver_grado_puesto(
      ...
      hn.cod_tipo_nomina, hn.denominacion, hn.denominacion_devengado
    FROM v_cnmd06_fichas_2 as f 
...
router.get("/sisap/empleado", async (req, res) => {
  ...
  try {
    const sqlQuery = `SELECT dp.cedula_identidad, dp.nacionalidad, dp.primer_apellido, dp.segundo_apellido,
       dp.primer_nombre, dp.segundo_nombre, dp.sexo, dp.fecha_nacimiento, ct.denominacion_clase as cargo, ct.puesto_grado as grado, f.fecha_ingreso, f.funciones_realizar, f.cod_ficha, f.cod_tipo_nomina, hn.denominacion as nomina, f.cod_cargo,
       CASE
...
router.post("/sisap/solicitud_recurso/guardar", express.json(), async (req, res) => {
  ...
  try {
    ...
    const numero_compromiso = res_numero[0].numero_compromiso;
    ...
    for (let index = 1; index <= id_send; index++) {
      ...
      const monto_partida = req.body["monto_solicitar_" + index];
    ...
    await specificQuery({ sqlQuery: sqlQuery_i_partidas, db });

    const sqlQuery_i_update_cfpd05 = sql_update132;
    await specificQuery({ sqlQuery: sqlQuery_i_update_cfpd05, db });

    const sqlQuery_i_insert_cfpd21 = sql_insert_cfpd21;
    await specificQuery({ sqlQuery: sqlQuery_i_insert_cfpd21, db });
...
router.get("/sisap/consulta_dep/:cod_dep", async (req, res) => {
  const { cod_dep } = req.params;
  ...
  const getCondition = () => {
    return `f.cod_dep=${cod_dep} and hn.clasificacion_personal in (1,2,3,4,5,16,12,17,18)`;
  };
